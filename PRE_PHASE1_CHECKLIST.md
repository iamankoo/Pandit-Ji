...
2. ~~Resolve repository/service naming conflicts between `Phases.md` and `ARCHITECTURE.md`.~~ **RESOLVED — LOCKED**: canonical services are `astro-engine`, `rule-engine`, `agent`, `knowledge`, `verification`; canonical apps are `mobile`, `web`, `admin`; canonical packages are `shared`, `contracts`, `ui`. See `Phases.md` Phase 3 and `docs/ARCHITECTURE.md` §27. The FastAPI HTTP layer's location (no `apps/api` entry in the locked list) was fully resolved in Phase 2 via ADR-007: it lives in a new top-level `server/` directory — see `docs/ARCHITECTURE.md` §27 and `docs/architecture/adr/ADR-007-api-gateway-and-fastapi-composition-root.md`.
