...
- Rule use goes only through `pandit_rule_engine.shadbala_gate.shadbala_for_rule`: a total only from a complete `MODERN_RAMAN` result with an exact birth time; otherwise one precise reason. No strength threshold is locked, so no shipped rule calls it; no shipped rule contains a strength condition (corrected in the §38 audit; any future one returns `requires_shadbala`).
...
## 37. Phase 10 Owner Decisions and Acceptance (2026-09-25) — Continuation Point

**Status: Phase 10 is accepted by the owner as complete for its supported scope** (`Phases.md` Phase 10 status block; `docs/ASTROLOGY_STANDARDS.md` v1.24.0).

### A. Owner decisions (recorded in the standards, registry and roadmap)
1. **Saura frame (PC-15)**: Lahiri is the default for user-facing month facts (present-day almanac practice); the CRC 23 deg 15 min frame is an explicitly labelled alternative, selectable per request. Every result records the selected frame, its month facts and the month facts of both frames, with provenance. Implemented: `PanchangRequest.saura_frame`, `DailyPanchang.saura_frame` and `lunar_month`; profile `PANCHANG_DRIK_CRC_1955_V2`; the rule-engine `panchang` section records the selected frame.
2. **Muhurta rules (MU-03)**: stay as astro-engine data; no migration to rule YAML for structural consistency alone; the deviation is documented. `export_rules()` gives the whole rule set as one versioned JSON document (provenance, stable IDs, statements) for audit, explanation or later migration.
3. **Choghadiya and Gowri (PC-22)**: deferred and unavailable until adequately verified primary sources exist; secondary websites are not an authority.
4. **Muhurta purposes (MU-01)**: no expansion until Vivaha, Griha Pravesha and Chaula are validated in production.
5. **Shadbala thresholds (SM-11)**: no universal "strong" threshold; Shadbala-dependent rules stay `NOT_EVALUABLE` until a separately researched and approved threshold methodology exists.

### B. Deviations and deferred items (unchanged in substance)
- Deviation: Muhurta rules outside the Phase 6 rule format (MU-03).
- Deferred: Choghadiya, Gowri, unequal horas, Bhadra residence, regional solar calendars, Karttikadi/Ashadhadi Vikrama and current Saka years, other Muhurta purposes, heliacal windows for marriage, the Kalaprakasika month rule for marriage.
- Roadmap note: Griha Pravesh and Housewarming are one ceremony and one rule set.
- Versions: standards v1.24.0; astro-engine and rule-engine 0.13.0.

### C. Next phase (Phase 11, Numerology + Compatibility): plan prepared, not approved
The plan (scope, source requirements, dependencies, decisions) was delivered to the owner with this record. Summary of the genuine decisions it raises: (1) which matching system (the North Indian 8-kuta, 36-point Ashtakoot has no source read yet -- Muhurta Chintamani has no English text read; Kalaprakasika Ch. XIII read in Phase 10 describes the South Indian ten-porutham system, counted from the bride's nakshatra); (2) both systems are role-asymmetric (bride and groom), and the product does not collect gender -- whether to add an explicit, non-gender "matching role" input needs a product-policy and privacy decision, otherwise the asymmetric factors are `NOT_EVALUABLE`; (3) minors: age controls for marriage matching (`PRODUCT_POLICIES.md` Minors); (4) numerology sources and defaults: the Chaldean and Pythagorean letter tables, master-number retention (no default by standard), Devanagari and Hinglish names (the tables are for the Latin alphabet; transliteration is a decision); (5) whether "numerology interpretation" and "lucky numbers" (interpretive content) are in scope before a source is read.

### D. Commits, push and CI
- Commit `1eab8be` feat: apply the Phase 10 owner decisions (standards v1.24.0) (author and committer `iamankoo <aniketraj00384@gmail.com>`, no AI attribution); plus this documentation commit. Push `f3b863c..1eab8be`; `origin/main` verified equal to `1eab8be1412d968ea7b2b424782cc6172e8d6c84`.
- Local: astro-engine 1563 passed (1561 + 2: default-frame test, rules export test); rule-engine 363 passed; lint, format, mypy and integrity scans clean.
- CI run `36137677558` (commit `1eab8be`): conclusion success; 15 of 15 jobs; all 173 steps inspected, 13 skipped by design (the same steps as before), none failed; astro-engine job 1563 passed; rule-engine job 347 passed, 6 skipped (module-level Swiss Ephemeris skips).

## 38. Phase 10 Closure Audit (2026-09-25)

Audit only, on the owner's instruction; no feature added and Phase 11 not started. Base: `bf832f9`.

### A. Verified
1. **Saura frame**: `PanchangRequest.saura_frame` defaults to `lahiri_variable` (`DEFAULT_SAURA_FRAME`); `crc_fixed_23_15` is selectable; every result carries `saura_frame`, `lunar_month` and both frames' `lunar_months`; provenance `prov.saura_frames` (`modern_tradition`) and `prov.month` (CRC page image); standards PC-15, registry conflicts and profile table, `Phases.md` and README agree.
2. **Muhurta rules**: astro-engine data documented as an owner-approved deviation (standards MU-03, `Phases.md`, SUMMARY §37); `export_rules()` returns `rules_version` 1.0.0, every rule's ID, statement, evidence label and source reference, and the aliases (tested).
3. **Deferred items not exposed**: Choghadiya, Gowri, unequal horas, regional solar calendars, Bhadra residence and regional Vikrama years appear only in each Panchang's `not_implemented` list; Muhurta offers only three purposes (plus aliases of them); no server, app or rule code exposes a Panchang or Muhurta feature.
4. **Shadbala**: `shadbala_for_rule` is called by no rule and no code path; no threshold exists in code; no shipped rule contains a strength condition.
5. **Roadmap mismatches**: Griha Pravesh = Housewarming (MU-02, `Phases.md`, §36-§37); Muhurta rule location (MU-03); the strength claim (see B).
6. **Tests and CI**: local astro-engine 1563 passed, rule-engine 363 passed; CI runs 36135679187 (`23eeb01`), 36135986125 (`f3b863c`), 36137677558 (`1eab8be`) and 36138034833 (`bf832f9`) all success, 15/15 jobs.
7. **No unapproved change**: no Phase 11 package or code; `Phases.md` changed only by the additive Phase 10 status block; `features.md`, `PRODUCT_POLICIES.md`, `LEGAL_REGULATIONS.md`, `SOURCE_OF_TRUTH.md`, `TECH_STACK.md` and the rule YAML untouched since `5c3415f`; every deleted line in that range is an in-place edit of a header or version line.

### B. Findings corrected (documentation only)
- Standards SM-11, SUMMARY §36.A and the `shadbala_gate` docstring said "the Phase 6 strength rules keep `requires_shadbala`", implying such rules exist; none does. Corrected, and §Strength clarified (Shadbala exists, no threshold). Editorial; no version change.
- `docs/ARCHITECTURE.md` module tree said "lunar months (both saura frames)" without the default and did not record the MU-03 deviation; corrected.
...
## 39. Final Stopping Point (2026-09-25) — Continuation Point

**Development is stopped here on the owner's instruction.** Phase 10 closure is complete **only for its owner-approved, source-supported scope**. **Phase 11 has not started**: no Phase 11 code, research, methodology lock or roadmap change exists. The Phase 11 notes in §37.C are an unapproved outline, not a plan in force.

### A. How to resume
1. Reread this file (this section first) and `Phases.md` in full, then `docs/ASTROLOGY_STANDARDS.md` (v1.24.0) as needed.
2. Verify the repository: `git log`, `origin/main` equal to the handoff commit (the documentation commit after `3404976`, see F), a clean tree, and the last CI run green.
3. Do nothing further until the owner gives explicit approval for the next step; continue only from this stopping point.

### B. Implemented in Phase 10 (standards v1.23.0 and v1.24.0) and its limits
- **Panchang** (`PanchangService.daily`): the Hindu day from sunrise to the next sunrise; every Tithi, Nakshatra, Yoga and Karana with exact start and end instants (kshaya and repeated elements flagged); Vara; sunrise and sunset (default: Sun's centre with 30 minutes of refraction, Calendar Reform Committee 1955; alternative: the Phase 4 upper-limb convention); moonrise and moonset (upper limb, engineering convention); equal one-hour horas; Rahu Kalam (a translator's note in Kalaprakasika), Yamaganda and Gulika Kalam as eighths of the actual day and night (proportional division is an inference); Nakshatra Panchaka and the remainder Panchaka (two different things, never merged); Bhadra as the Vishti karana. Limits: the only regional configuration is the pan-Indian Drik convention of the CRC; polar days without sunrise or sunset are `NOT_EVALUABLE`.
- **Calendar**: amanta and purnimanta months, adhika and suppressed (kshaya) months, Saka and Chaitradi Vikrama years (expired); years are withheld when a year would begin with an adhika Chaitra. Golden test: the CRC's printed calendar for 22 March - 21 May 1954 (61 days, 16 phenomena) within one minute, one printed slip recorded.
- **Special points** (`SpecialPointsService`): Sun-based upagrahas (BPHS verse and translator-note readings), Gulika and Mandi (three readings), Bhava, Hora, Ghatika and Varnada Lagnas, Pranapada (Sun at the given time and at sunrise). Ghati = 24 minutes (engineering convention).
- **Tara Bala and Chandra Bala**: Tara from the birth nakshatra; Chandra Bala in two profiles (Kalaprakasika: 8th only; Raman: 6th, 8th, 12th); both need the person's birth nakshatra and Moon sign, otherwise `NOT_EVALUABLE`.
- **Muhurta** (`MuhurtaService.evaluate` and `.search`, up to 31 days): Vivaha, Griha Pravesha (= Housewarming) and Chaula (= Mundan) from Kalaprakasika, with Raman's Tarabala, Chandrabala and Panchaka; factor facts with sources, no overall verdict, no statements of effect; overlapping statements in one chapter reported as separate factors; inputs the product does not collect (child's age, pregnancy, a partner's role, entrance direction, heliacal visibility, month system) are `NOT_EVALUABLE`.
- **Rule engine**: optional, transport-only `panchang` evidence section recording the selected saura frame; no rule reads it.

### C. Deferred and excluded
- **Deferred (owner decisions)**: Choghadiya and Gowri Panchangam (until adequately verified primary sources exist; websites are not an authority); additional Muhurta purposes (until the three are validated in production); a Shadbala "strong" threshold.
- **Not implemented (no source or out of scope)**: unequal horas; Bhadra residence rules; regional solar calendars (Tamil, Bengali, Malayalam, Odia); Karttikadi and Ashadhadi Vikrama years and current Saka years; the marriage month rule (month system unspecified) and heliacal windows.
- **Excluded by policy**: statements of effect in the Muhurta sources, the marriage chapter's child-age rule and caste-specific exceptions (`PRODUCT_POLICIES.md`).
- **Earlier phases (unchanged)**: Phase 9 deferrals (Lal Kitab, Nadi, general Horary, Vastu, Feng Shui, Jaimini Dashas, Tarot meanings) and Ayurdaya excluded (§35).

### D. Final decisions in force
- **Shadbala (v1.22.0, SM-01 to SM-12)**: `MODERN_RAMAN` (the Raman profile) is the default user-facing method; `BPHS_VERSE_REFERENCE` stays a reference method without a total; never combined; every result carries method, version, sources, assumptions, choices, confidence, not-evaluable components and complete or partial totals; Raman's two contradictions are caller-configured or reported as unresolved with the alternatives' totals, never defaulted; rule use only through `shadbala_for_rule`. No universal "strong" threshold: any rule that would need one stays `NOT_EVALUABLE(requires_shadbala)`; no shipped rule contains a strength condition.
- **Saura frame (PC-15)**: Lahiri is the default for user-facing month facts; the CRC 23 deg 15 min frame is an explicitly labelled alternative, selectable per request; every result records the selected frame and the month facts of both frames, with provenance.
- **Muhurta architecture (MU-03)**: the rules stay as versioned astro-engine data (`MUHURTA_RULES_VERSION` 1.0.0, `export_rules()` for audit and future migration); this is an intentional, owner-approved deviation from the Phase 6 rule format, documented in the standards, `docs/ARCHITECTURE.md`, `Phases.md` and here.
- **Roadmap note**: Griha Pravesh and Housewarming are one ceremony and one rule set.

### E. Unresolved future decisions (owner)
- Phase 11 approval and, before any work, its source and policy questions: which matching system (no Ashtakoot source read yet; Kalaprakasika Ch. XIII is the South Indian ten-porutham system), role-asymmetric matching without collected gender, age controls for minors, numerology defaults (letter table, master numbers, Devanagari and Hinglish names), and whether interpretive numerology is in scope.
- A Shadbala strength-threshold methodology (separate research and approval).
- Sources for Choghadiya and Gowri; further Muhurta purposes after production validation.
- Earlier open items unchanged (for example the Raman readings, the KP day-lord convention, the Chinese day boundary).

### F. Final commit and CI
- Last feature commit: `1eab8be` (Phase 10 owner decisions, standards v1.24.0); last audit commits: `47c140d` and `3404976`.
- Verified CI before this record (all success, 15 of 15 jobs, every step inspected, only the 13 by-design skipped steps): `36135679187` (`23eeb01`), `36135986125` (`f3b863c`), `36137677558` (`1eab8be`), `36138034833` (`bf832f9`), `36141007597` (`47c140d`), `36141319088` (`3404976`).
- The handoff commit that adds this section is the next commit after `3404976` on `origin/main` (a commit cannot contain its own hash); its hash and CI run were verified and reported to the owner at the end of the session. Confirm them with `git log -1` and `gh run list --limit 1` when resuming.
- Final local test counts: astro-engine 1563 passed; rule-engine 363 passed (CI: 347 passed, 6 module-level Swiss Ephemeris skips).
