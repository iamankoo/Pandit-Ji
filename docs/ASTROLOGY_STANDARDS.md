...
## Ashtakavarga standards (Phase 9 WP-A1 methodology lock, v1.7.0)

Scope: Bhinnashtakavarga (per-planet, 12-sign benefic-contributor counts) and Sarvashtakavarga (their sum across the seven planet charts) as structural facts only. No interpretation, verdict, remedy, strength label ("strong"/"weak") or narration. Out of scope, deferred, not silently dropped: Trikona Shodhana, Ekadhipatya Shodhana and Pinda Sadhana reductions (WP-A2/A3, blocked on a printed internal conflict over the Pinda planet multipliers and on transcribing the reduction worked examples); any use of Ashtakavarga by Shadbala's Drik Bala or by Gochara/transits (Phase 8, which explicitly excluded Ashtakavarga scoring); Rahu and Ketu, which are never contributors because no source read (Brihat Jataka Ch. IX, Phaladeepika Ch. XXIII, BPHS Ch. 66) gives either node a table.
...
Affected calculations: new only (Ashtakavarga Bhinna and Sarva facts). No Phase 4, 5, 6, 7 or 8 formula changed. Affected interpretations: none; later phases (or a future WP-A2 reduction package) consume the facts. Regression-test requirement: the full 56-cell source-comparison fixture and 9-cell conflict register, the Brihat Jataka worked-example fixture, rotation-arithmetic and invariant tests, per-profile Lagna-chart presence/absence tests, and every failure path (`services/astro-engine/tests/test_ashtakavarga_source_comparison.py`, `test_ashtakavarga_calculator.py`, `test_ashtakavarga_service.py`). Approval status: the four-profile design (no winner chosen, no silent correction, every printed reading preserved exactly, the 9 disagreement cells carried with an `unresolved_conflict`-equivalent register, the Lagna chart profile-dependent and never synthesized) was approved by the project owner at Phase 9 Gate 1 following an independent reconciliation audit of the comparison data.
