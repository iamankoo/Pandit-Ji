...
- **Method policy (v1.22.0)**: `ShadbalaMethodService` selects one method per request -- `MODERN_RAMAN` (the default for user-facing Shadbala) or `BPHS_VERSE_REFERENCE` -- and never combines them. Each result reports method, version, sources, assumptions, methodology choices, confidence, not-evaluable components and complete or partial totals. A Raman reading the caller leaves unset is not defaulted: components it changes become `NOT_EVALUABLE(methodology_reading_not_selected)` and the totals under every option are returned (SM-01 to SM-12).
...
## Panchang, special points and Muhurta (Phase 10)

`PanchangService.daily` (`pandit_astro_engine.panchang`) returns one Hindu day -- the civil date's sunrise to the next sunrise at the location, with an explicit IANA timezone -- as `DailyPanchang`: Vara; every Tithi, Nakshatra, Yoga and Karana current during the day with exact start and end instants (kshaya and repeated elements flagged); sunrise, sunset, moonrise and moonset; the amanta and purnimanta month, adhika and suppressed months and the Saka and Chaitradi Vikrama years with the Lahiri saura frame by default and the Calendar Reform Committee's fixed 23 deg 15 min frame as a labelled alternative (selectable per request; the month facts of both frames always reported); equal horas; Rahu Kalam, Yamaganda and Gulika Kalam; the Nakshatra Panchaka; Bhadra (Vishti). The default configuration follows the Calendar Reform Committee (1955): sunrise from the Sun's centre with 30 minutes of refraction; the Phase 4 upper-limb sunrise is an explicit alternative. The printed calendar of the Committee for 22 March - 21 May 1954 is the golden test (`tests/fixtures/crc_1955_calendar_saka_1876.json`, within one minute). Choghadiya and Gowri Panchangam are not implemented (no primary source read) and are listed in `not_implemented`; they stay deferred until verified primary sources exist (owner decision). `muhurta.export_rules()` returns the Muhurta rule sets as one versioned JSON document.
